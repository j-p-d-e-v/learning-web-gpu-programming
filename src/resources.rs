...
pub async fn load_string(file_name: &str) -> anyhow::Result<String> {
    ...
    let txt = {
        let env_out = std::env::var("OUT_DIR").unwrap_or("storage".to_string());
        let path = std::path::Path::new(&env_out).join("res").join(file_name);
...
pub async fn load_binary(file_name: &str) -> anyhow::Result<Vec<u8>> {
    ...
    let data = {
        let env_out = std::env::var("OUT_DIR").unwrap_or("storage".to_string());
        let path = std::path::Path::new(&env_out)
        .join("res").join(file_name);
        std::fs::read(path)?
    };
    Ok(data)
}
